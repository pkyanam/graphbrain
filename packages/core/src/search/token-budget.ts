// @graphbrain/core — token budget enforcement on search results (Stage 9).
//
...
// stops when the next result would push the running total past the budget.
//
...
// estimating in code (which is what we want for a safety budget).
//
// Backward-compatibility: when no budget is set (undefined or <= 0), the
// enforcer is a no-op. tokenmax mode sets tokenBudget = undefined.
//
// Ported from _reference/gbrain/src/core/search/token-budget.ts. Adapted
// to Graphbrain's SearchResult shape: the per-result token cost is
// page.title + sum(chunks[].content) (GBrain used title + chunk_text).
//
 ...
 * prose; closer to 3 for code with lots of punctuation; up to 8 for CJK.
 * Overshoot is fine for a safety budget. Undershoot would let us blow
 * past the cap, so we round UP when in doubt.
 */
...
/**
 * Per-result token cost: page title + chunk contents. Slug is metadata
 * and doesn't enter the assistant context, so we don't count it.
 */
export function resultTokens(r: SearchResult): number {
  let total = estimateTokens(r.page.title);
 ...
 *
 * Edge cases (all preserve the no-budget contract):
